...
# 
# $\frac{df(x)}{dx} = \lim_{\delta x \to 0} \frac{f(x+\delta x)-f(x)}{\delta x}$, where, without going into detail $f(x)$ has to be differentiable. For example, if $f(x)=x^2$ we find
# 
...
#  
#  We also find that the $\frac{\Delta x^2}{2!} f''(x)$ term in the error term cancels in this expression so that in this case the error term is of the order $(\Delta x)^2$ rather than $\Delta x$. Hence the _central_ difference method is considerably more accurate for a given $\Delta x$ and this method would be used in preference.
#  
